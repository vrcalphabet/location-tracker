...
**Location Tracker** は、リアルタイムで特定のユーザの位置情報を共有・追跡するためのWebアプリケーションです。ユーザが5分ごとに位置情報をサーバに送信することで、メッセージアプリ等でやり取りすることなく居場所を地図上に可視化します。
...
- `GET_TOKEN`
  端末データ取得用、リバースジオコーディング取得用、メインページ(`/view/`)のアクセス用に使用するトークンです。任意の文字種で32桁以上のランダムな文字列を生成し、それをSHA256でハッシュ化したものを指定します。なお、このトークンはURLのGETパラメータに入れるため、URIエンコードがされない文字種を使用することを推奨します。
...
ブラウザで `http://localhost:5173/view/` にアクセスすれば画面が表示されるはずです。
